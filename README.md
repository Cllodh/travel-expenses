...
- **匯率來源切換**：改用 UniRate API，取得更穩定、即時的匯率資料。
- **Google Sheets 流程優化**：修正多筆連續寫入時的同步問題，確保每次送出都能正確寫入 Google 試算表。
- **動態提示訊息**：寫入成功時，提示「已成功將 日期 國家之旅程的花費寫入 Google 試算表！」，內容依填寫資料自動變化。
- **金額欄位防呆**：所有花費明細欄位禁止輸入負數，前端自動移除負號，並強制總花費不得為負數。
- **UI/UX 微調**：首頁說明區塊美化，說明內容更清楚，並可隨時顯示不自動消失。
- **bug 修正**：
  - 修正表單送出後無法連續新增的問題。
  - 修正 Google Sheets ID 取得失敗時的提示與流程。
  - 修正部分欄位 reset 後未自動補回隱藏欄位的問題。
- **程式碼重構**：前端 JS 狀態管理更穩定，localStorage 與隱藏欄位同步機制加強。
- **多筆金額自動加總**：機票、住宿、SIM、票券、保險等欄位支援多筆金額（逗號分隔），後端自動加總，方便分段記帳。
- **必填欄位提示優化**：國家、出發日、回國日欄位標示更明顯的紅色星號，並加強前端驗證，未填寫時無法送出。
- **金額欄位防呆再強化**：所有金額欄位禁止輸入負號，並自動移除，確保資料正確。
- **Google Sheets 寫入穩定性提升**：修正多筆金額、欄位型態等特殊情境下寫入失敗的問題。
- **UI/UX 細節微調**：必填欄位標示、錯誤提示、表單互動體驗更直覺。
